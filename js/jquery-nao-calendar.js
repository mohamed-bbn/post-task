    ...
    select: function(date) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $(".lessonByDate").empty();
                if (data.length == 0) {
                    $(".lessonByDate").append(`<div class="img-text"><span>There is no scheduled lesson</span></div>`)
                } else {
                    $.each(data, function(index, lesson) {
                        $(".lessonByDate").append(` <p class="text">` + lesson.date + `at ` + lesson.start_time + `.</p>
                            <div class="img-text"><span>Lesson scheduled with</span>
                                <div class="img"><img src="` + '/images/user_img/'.concat(lesson.user_img) + `" alt="Arabia" title="Arabia">
                                    <p>` + lesson.fname + `  ` + lesson.lname[0] + `.</p>
                                </div>
                            </div>`)
